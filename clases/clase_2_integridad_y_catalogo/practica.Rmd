...
>
> **Cómo repartirlo:** cada pareja elige **una** de las secciones 1 a 3 (30'). La sección 4 se
> hace entre todos al final (10'), porque el resultado es una decisión de proyecto.
>
...
Monitor Mensual de Empresas. **Nunca se auditó** con los criterios que vimos hoy. Vamos a
hacerlo.
...
Tres tests: (1) contraste de luminancia, (2) distancia perceptual ΔE2000, (3) simulación de
daltonismo. **La discusión final es una decisión de proyecto**, no un ejercicio.
...
La luminancia (el canal "L" del espacio Lab) es lo que hace que una serie salte a la vista. Va de
0 (negro) a 100 (blanco).
...
**Pregunta:** ¿cuánta diferencia de luminancia hay entre La Rioja y las otras dos? ¿Y entre "Resto
país" y "NOA-Resto" entre sí? ¿Es eso deliberado o accidental?

## Test 2 — Distancia perceptual (ΔE 2000)

ΔE mide cuán distintos se PERCIBEN dos colores. Regla práctica: ΔE < 5 → prácticamente
indistinguibles; ΔE < 10 → riesgoso para series superpuestas; ΔE > 20 → cómodamente distinguibles.
...
~8% de los varones tiene alguna forma de daltonismo. Si dos series de un gráfico se vuelven el
mismo color, ese lector no puede leerlo.

```{r ej4-test3, eval=TRUE}
...
**TODO:** calculá el ΔE de la paleta simulada bajo deuteranopia, igual que en el Test 2, y
compará con la matriz de arriba. ¿Algún par baja del umbral?
...
**Pregunta:** en las simulaciones, ¿podés seguir distinguiendo las tres series? ¿Y encontrar La
Rioja?
...
- **A) Dejarla como está.** El contraste de luminancia de La Rioja sobrevive a cualquier tipo de
  daltonismo (la luminancia no depende del canal cromático), y La Rioja es lo único que TIENE que
  destacarse.
- **B) Ajustar** los dos colores de contexto para separarlos más entre sí. Costo: nos alejamos de
  la identidad visual del Monitor de Fundar.
- **C) Agregar un canal redundante:** que La Rioja además tenga la línea más gruesa. Así el
  énfasis no depende SOLO del color. (El tema viejo, `style/fundar_larioja_theme.R`, ya tiene
